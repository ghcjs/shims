...
(function(){
	...
	Chart.Type.extend({
			...
			helpers.each(valuesArray,function(value,datasetIndex){
					...
					//Add a new point for each piece of data, passing any required data to draw.
						this.datasets[datasetIndex].points.push(new this.PointClass({
							value : value,
							label : this.datasets[datasetIndex].label,
		...
		hideDataset : function(id) {
			if (typeof(id) === 'number' && id > -1 && id < this.datasets.length) {
				this.datasets[id].hidden = true;
			} else {
				for (var index = 0; index < this.datasets.length; ++index) {
					if (this.datasets[index].id === id ) {
						this.datasets[index].hidden = true;
						break;
					}
				}
			}
		...
		showDataset : function(id) {
			if (typeof(id) === 'number' && id > -1 && id < this.datasets.length) {
				this.datasets[id].hidden = false;
			} else {
				for (var index = 0; index < this.datasets.length; ++index) {
					if (this.datasets[index].id === id ) {
						this.datasets[index].hidden = false;
						break;
					}
				}
			}
